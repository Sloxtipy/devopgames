function validateUser(isDoctor){
	var ciDrf = jQuery("#cidrc").val();
	var ciDr = ciDrf.length;
    var espacio_blanco    = /\s/;
    var campo_validar = jQuery("#usrLogin").val();
	...
	if (isDoctor==1){
			...
			else if (jQuery("#cidrc").val()==""){
			showMessageErr2("Alerta: Debe llenar el campo Cédula"); return false
			}
			else if (jQuery("#cidrc").val()==""){
				showMessageErr2("Alerta: Debe llenar el campo Cédula"); return false
			}
			else if (ciDr > 10){
				showMessageErr2("Alerta: Número de Cédula inválido."); return false
			}
			else if (ciDr <= 5){
				showMessageErr2("Alerta: Número de Cédula inválido."); return false
			}
			...
			showMessageErr2("Alerta: Debe seleccionar el País"); return false
			}
			else if (jQuery("#drCountryClinic").val()=="-"){
			showMessageErr2("Alerta: Debe seleccionar el País"); return false
			}
			else if (jQuery("#drCityClinic").val()=="-"){
			showMessageErr2("Alerta: Debe seleccionar la Ciudad"); return false
			}
			else if (jQuery("#clinic").val()=="-"){
			showMessageErr2("Alerta: Debe seleccionar la Clínica"); return false
			}
				} else {
                        ...
                        if(espacio_blanco.test(campo_validar)){
                            showMessageErr("Alerta: No se permiten espacios en blanco para el campo USUARIO."); //jQuery("#usrLogin").focus();
                            return false
                        }
						...
						//  else if (jQuery("#ci").val()==""){
						// 	 		showMessageErr("Alerta: debe llenar el campo CEDULA"); //jQuery("#ci").focus();
						//  return false
						// }
						...
						// else if (jQuery("#Uphone").val()==""){
						// 		showMessageErr("Alerta: debe llenar el campo TELEFONO");// jQuery("#Uphone").focus();
						//  return false
						// }
						else if (jQuery("#address").val()==""){
								showMessageErr("Alerta: debe llenar el campo Ciudad");// jQuery("#address").focus();
						 return false
						}
						...
						else if (jQuery("#CountryClinic").val()=="-"){
									showMessageErr("Alerta: Debe seleccionar el pa&iacute;s");// jQuery("#ptcountry").focus();
						 return false
						}
						else if (jQuery("#CityClinic").val()=="-"){
									showMessageErr("Alerta: Debe seleccionar la Ciudad");// jQuery("#ptcountry").focus();
						 return false
						}
						else if (jQuery("#babyClinic").val()=="-"){
									showMessageErr("Alerta: Debe seleccionar la Clínica ");// jQuery("#ptcountry").focus();
						 return false
						}
						else if (jQuery("#babyDoctor").val()=="-"){
									showMessageErr("Alerta: Debe seleccionar el Doctor");// jQuery("#ptcountry").focus();
						 return false
						}
						// else if ((jQuery("#ptsex1").is(':checked')==false)&&(jQuery("#ptsex2").is(':checked')==false)){
...
function findCitiesPt(idCountryPt,selectNamePt){
	...
	var valIdCity = Number(idCountryPt);

	if (idCountryPt!="-" && valIdCity !== -3){
		jQuery("#ajaxLoader").fadeIn();
		jQuery.ajax({
			type: 'GET',
			dataType: 'text',
...
function displayEmptyClinicsPWGB() {
	deleteOptions("CityClinic");
	addObjToCombo("CityClinic", "109", "-ASIGNADO POR PEDIATRAWEB-");
	deleteOptions("babyClinic");
	addObjToCombo("babyClinic", "30", "-ASIGNADO POR PEDIATRAWEB-");
	deleteOptions("babyDoctor");
	addObjToCombo("babyDoctor", "1641", "PEDIATRAWEB, ASIGNADO");
}
